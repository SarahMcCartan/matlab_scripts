...
for lag = a:b:c
    ...
    fout1 = fullfile(OutFolder, sprintf('%d_%s_Count_Matrix_TBA_%07d.txt',temp,pep,lag)); 
    dlmwrite(fout1, CM, 'delimiter', '\t', 'precision','%.5f');
    
    CM=zeros(ns,ns); %reset the CM for next loop

end

fout2 = fullfile(OutFolder, sprintf('%d_%s_lagVals.txt',temp,pep));
dlmwrite(fout2, lagVals, 'delimiter', '\t');

%save out the index per transition to check sliding window is correct
