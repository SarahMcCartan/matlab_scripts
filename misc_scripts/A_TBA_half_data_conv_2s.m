...
f_in='450_dcsa_pca_12m.txt';
PCA_data1=load(f_in,'-ascii') ; 
%PCA_data = PCA_data1(1:(length(PCA_data1))/2,:);
PCA_data = PCA_data1((length(PCA_data1))/2:length(PCA_data1),:);

%define global variables
temp = 450;   %temperature of simulation
pep = 'dCSA_2half'; %csa or dcsa peptide string
lag = 1;      %lag time
ns = 2;

%LB method uses lag time, have defined the lag time here, set to 1 for now.
%define lag time in n steps which is n*500ps (time between each step)
n_iter = 20;   %use this for file names

R = 0.44;
%n_iter = n_iter +1; %update file name for each iteration i.e. each R value
...
%the centre of each state cluster
	...
	%plot(C4(:,1)+ R*cos(t), C4(:,2)+ R*sin(t),'m-')
	%r4 = hypot(PCA_data(:,1)-C4(:,1), PCA_data(:,2)-C4(:,2));

	%plot(PCA_data(r4<=R,1),PCA_data(r4<=R,2),'mo')
	...
	fname1 =  sprintf('R%d_%d_%s_rvals.txt',n_iter, temp,pep);
	...
	%create 4 x 4 count matrix test
	...
	fname2 =  sprintf('R%d_%d_%s_count_matrix.txt',n_iter,temp,pep);
	fname3 =  sprintf('R%d_%d_%s_state_tba.txt',n_iter,temp,pep);
	...
	fname4 =  sprintf('R%d_%d_%s_S1_frames.txt',n_iter,temp,pep);
	fname5 =  sprintf('R%d_%d_%s_S2_frames.txt',n_iter,temp,pep);
	%fname6 =  sprintf('R%d_%d_%s_S3_frames.txt',n_iter,temp,pep);
	%fname7 =  sprintf('R%d_%d_%s_S4_frames.txt',n_iter,temp,pep);
	...
	fname8 =  sprintf('R%d_%d_%s_TBA_check.txt',n_iter,temp,pep);
	fname9 =  sprintf('R%d_%d_%s_TBA_states.txt',n_iter,temp,pep);
	...
	CM=zeros(ns,ns);
	%define RRtime in n steps which is n*500ps (time between each step)
	lag_t=1;
	iter_t = 0;
	...
	fname11 =  sprintf('R%d_%d_%s_count_matrix_TBA.txt',n_iter,temp,pep);
	fname12 =  sprintf('R%d_%d_%s_Life_T_Avg.txt',n_iter,temp,pep);
