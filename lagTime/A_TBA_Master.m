...
R = 0.15;
ns = 3;
t_step = 500e-12; %500ps = size of time step
...
%create a new r_values matrix with just those relevant to the lag time
%pulled out 

r_vals2 = zeros(round(lr/lag), ns);

idx = 0;

for ii=1:lag:length(r_values)
    disp(ii)
    idx = idx+1;
    r_vals2(idx,:) = r_values(ii,:);
...
State_TBA=(1:length(r_vals2)); %state array
...
%use the logical indices to index into VLM_state to return required sub
%matrices
%this returns the Lifetime Distribution of each state!!

S1= VLM_state(ind1,:);
S2= VLM_state(ind2,:);
S3= VLM_state(ind3,:);
%S4= VLM_state(ind4,:);
fnames1 =  sprintf('%d_%s_Life_T_S1_%d.txt',temp,pep,lag);
fnames2 =  sprintf('%d_%s_Life_T_S2_%d.txt',temp,pep,lag);
fnames3 =  sprintf('%d_%s_Life_T_S3_%d.txt',temp,pep,lag);

dlmwrite(fnames1, S1, 'delimiter', '\t');
dlmwrite(fnames2, S2, 'delimiter', '\t');
dlmwrite(fnames3, S3, 'delimiter', '\t');

%sum frames for each state to get Avg Lifetime
...
Life_T=[T1 T2 T3]; %average lifetime of each state
...
time = linspace(0,length(states)*conv*lag,length(states));
%time = 
t = time';
